...
5. OpenAPI есть токены, но нет эндпоинтов - как быть с авторизацией? Генерацию токенов сделать, но токены будут
   ...
   принял такое решение. Прошу учесть, что я прекрасно понимаю, что так делать - плохо. Вечные токены сделаны для
   облегчения проверки проверяющим. Нужно сделать эндпоинт, где можно генерировать JWT токен. Однако, исходя из
   OpenAPI это не понятно. Только один эндпоинт имеет явно возвращаемый код 401, хотя защищенными обозначены практически
   все!
